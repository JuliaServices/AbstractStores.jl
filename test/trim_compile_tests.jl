...
_trim_enabled() = get(ENV, "ABSTRACTSTORES_RUN_TRIM_COMPILE", "1") == "1"
...
function _setup_trim_environment()
    ...
    setup = "import Pkg; Pkg.develop(path=$(repr(package_path))); Pkg.add(\"JuliaC\")"
...
function _run_trim_case(project_path::String)
    script_path = joinpath(@__DIR__, "abstractstores_trim_store.jl")
    julia = joinpath(Sys.BINDIR, Base.julia_exename())
    mktempdir() do directory
        output_name = Sys.iswindows() ? "abstractstores_trim.exe" : "abstractstores_trim"
...
@testset "Trim compile" begin
    if !_trim_enabled()
        println("[trim] skip: ABSTRACTSTORES_RUN_TRIM_COMPILE != 1")
        @test true
